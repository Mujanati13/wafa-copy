...
import { BookOpen, ChevronDown, CircleAlert, Crown, Medal, Play, Star, TrendingUp } from "lucide-react";
  ...
  return <div className="space-y-7">
    <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-sky-100 via-blue-100 to-cyan-100 px-5 py-7 text-slate-900 shadow-xl shadow-blue-200/40 sm:px-8 sm:py-9 dark:from-[#10265f] dark:via-[#133f80] dark:to-[#12718d] dark:text-white dark:shadow-blue-950/15">
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
        <div><span className="hidden rounded-full border border-blue-300/70 bg-white/70 px-3 py-1 text-xs font-semibold tracking-[.12em] text-blue-700 uppercase dark:border-cyan-200/25 dark:bg-white/10 dark:text-cyan-100 sm:inline-flex">Espace de révision</span><h1 className="text-3xl font-bold tracking-tight sm:mt-4 sm:text-4xl">Bienvenue, {firstName}.</h1><p className="mt-2 max-w-xl text-sm leading-6 text-slate-600 sm:text-base dark:text-blue-100">Une petite session aujourd'hui construit de grands résultats demain.</p></div>
        <div className="hidden flex-col gap-3 sm:flex sm:flex-row"><Button onClick={() => navigate("/dashboard/subjects")} className="bg-primary text-primary-foreground hover:bg-primary/90"><Play className="mr-2 h-4 w-4 fill-current" />Continuer à réviser</Button><Button asChild variant="outline" className="border-blue-300 bg-white/70 text-blue-700 hover:bg-white hover:text-blue-800 dark:border-white/30 dark:bg-white/5 dark:text-white dark:hover:bg-white/10 dark:hover:text-white"><Link to="/dashboard/progress">Voir mes progrès</Link></Button></div>
      </div>
      <div className="relative mt-6 grid grid-cols-3 border-t border-white/70 pt-5 dark:border-white/15 sm:mt-8 sm:pt-6">
        <DashboardStat icon={<Medal className="h-6 w-6 sm:h-7 sm:w-7" aria-hidden="true" />} label="Classement" value={loading ? "…" : rank ? `#${rank}` : "—"} tone="amber" />
        <DashboardStat icon={<TrendingUp className="h-6 w-6 sm:h-7 sm:w-7" aria-hidden="true" />} label="Progression" value={loading ? "…" : `${semesterProgress}%`} tone="emerald" />
        <DashboardStat icon={<Star className="h-6 w-6 sm:h-7 sm:w-7" aria-hidden="true" />} label="Points" value={loading ? "…" : totalPoints.toLocaleString("fr-FR")} tone="gold" />
      </div>
    </section>
    ...
    {loading ? <DashboardSkeleton /> : <>
      ...
      <section id="modules" aria-label={`Modules ${selectedSemester || "disponibles"}`}>
        <div className="mb-5 flex items-center justify-between gap-4 border-b border-border pb-3">
          ...
          <SemesterPicker current={selectedSemester} semesters={userSemesters} onChange={setSelectedSemester} disabled={semesterLoading} />
        </div>
...
function SemesterPicker({ current, semesters, onChange, disabled }) {
...
function DashboardSkeleton() { return <div className="space-y-5"><div className="flex items-center justify-between"><Skeleton className="h-10 w-36 rounded-xl" /><Skeleton className="h-10 w-16 rounded-xl" /></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{[1, 2, 3, 4].map((item) => <Skeleton key={item} className="h-[230px] rounded-2xl" />)}</div></div>; }

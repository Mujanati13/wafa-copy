...
const runSeeder = async () => {
  ...
  console.log('====================================================');
  console.log('🚀 WAFA Full Database Seeder');
  ...
  await upsertDoc(Feedback, { name: 'Dr. Yassine E.', role: 'Interne en Médecine' }, {
    ...
    message: 'WAFA m\'a permis de réviser toutes les annales méthodiquement et de réussir mes examens de semestre avec mention.',
    rating: 5,
    isApproved: true,
    isFeatured: true,
    order: 1
  ...
  if (!existingSettings) {
    await LandingPageSettings.create({
      siteName: 'WAFA Medical Learning',
  ...
  if (!existingPolicy) {
    await PrivacyPolicy.create({
      content: 'Politique de confidentialité et protection des données personnelles de la plateforme WAFA.',
      termsOfUse: 'Conditions générales d\'utilisation et d\'accès aux services pédagogiques WAFA.',
